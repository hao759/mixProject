...
fetch(`${API}/${randomm}`).then(res=>res.json())
     .then(json =>hero.innerHTML =`<h1 style=" white-space: nowrap;">${json.biography["full-name"]}</h1><img src="${json.image.url}"/>`);
...
getHero.onclick=()=>{
    randomm=Math.floor(Math.random() * 731);
